...
def print_ledger(acc):
  ...
  if not acc.ledger:
    ...
    return
  for t in acc.ledger:
    print(f"{t.timestamp:%Y-%m-%d %H:%M:%S} | {t.kind:<12} | {pretty_money(t.amount, acc.currency)} | saldo={pretty_money(t.balance_after, acc.currency)} | {t.note}")
...
def export_ledger(acc, fmt: str, out_path: str):
  ...
  if fmt.lower() == "json":
    ...
    with open(out_path, "w", newline="", encoding="utf-8") as f:
      writer = csv.writer(f)
      writer.writerow(["timestamp", "kind", "amount", "balance_after", "note", "currency", "account_id", "account_type"])
      for t in acc.ledger:
        writer.writerow([
...
if __name__ == "__main__":
  ...
  parser.add_argument("--demo", choices=["bank", "contas", "both"], default="both",
                      help="Escolha qual demonstração executar")
  parser.add_argument("--list-accounts", action="store_true",
                      help="Lista contas (após demo_bank) com id, tipo e saldo")
  parser.add_argument("--extrato", metavar="ACCOUNT_ID",
                      help="Exibe o extrato (ledger) da conta informada (após demo_bank)")
  parser.add_argument("--export-extrato", metavar="ACCOUNT_ID",
                      help="Exporta o extrato (ledger) da conta informada (após demo_bank)")
  parser.add_argument("--export-format", choices=["csv", "json"], default="json",
                      help="Formato do arquivo de exportação (padrão: json)")
  parser.add_argument("--export-out", metavar="PATH", default="extrato.json",
    ...
    if args.demo in ("bank", "both"):
      ...
      if args.list_accounts or args.extrato:
        ...
        if args.extrato:
          ...
          if acc is None:
            print(f"Conta '{args.extrato}' não encontrada. Use --list-accounts para ver os ids.")
          else:
            print_ledger(acc)
      if args.export_extrato:
        acc = accounts.get(args.export_extrato)
        if acc is None:
          print(f"Conta '{args.export_extrato}' não encontrada. Use --list-accounts para ver os ids.")
        else:
